...
`edu_core` 是 `/scratch` 下教程共享的最小 PyTorch 基础包。它只包含没有论文策略含义的 token attention、ViT patch token、padding/label helpers、冻结/EMA 与 checkpoint state；不包含数据集、模型训练器、扩散 UNet 或某篇论文的损失。
